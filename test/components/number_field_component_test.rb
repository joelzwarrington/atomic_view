...
class AtomicView::Components::NumberFieldComponentTest < ViewComponent::TestCase
  ...
  test "renders number field with custom options" do
    ...
    expected = <<~HTML.strip
      <div class="relative rounded-lg shadow-xs">
        <input min="18" max="120" step="1" class="block w-full appearance-none h-9 min-w-0 z-10 flex-1 rounded-lg border-0 py-1 text-base shadow-xs ring-1 disabled:cursor-not-allowed disabled:bg-gray-50 disabled:text-zinc-500 disabled:ring-zinc-200 bg-transparent dark:bg-white/5 text-zinc-950 ring-zinc-950/10 dark:ring-white/10 placeholder:text-zinc-500 dark:text-white focus:ring-neutral-700 focus:border-zinc-950/20 dark:focus:ring-white/20" type="number" name="test_model[age]" id="test_model_age">
      </div>
    HTML
  ...
  test "renders number field with decimal step" do
    ...
    expected = <<~HTML.strip
      <div class="relative rounded-lg shadow-xs">
        <input step="0.01" class="block w-full appearance-none h-9 min-w-0 z-10 flex-1 rounded-lg border-0 py-1 text-base shadow-xs ring-1 disabled:cursor-not-allowed disabled:bg-gray-50 disabled:text-zinc-500 disabled:ring-zinc-200 bg-transparent dark:bg-white/5 text-zinc-950 ring-zinc-950/10 dark:ring-white/10 placeholder:text-zinc-500 dark:text-white focus:ring-neutral-700 focus:border-zinc-950/20 dark:focus:ring-white/20" type="number" name="test_model[price]" id="test_model_price">
      </div>
    HTML
