...
class AtomicView::Components::EmptyStateComponentTest < ViewComponent::TestCase
  test "renders the icon slot and title" do
    actual = render_inline(AtomicView::Components::EmptyStateComponent.new(title: "No messages")) do |component|
      component.with_icon { "<svg></svg>".html_safe }
    end.to_html
  ...
  test "does not render an icon when no icon slot is given" do
  ...
  test "renders block content as an action area" do
    actual = render_inline(AtomicView::Components::EmptyStateComponent.new(title: "No campers yet")) do
  ...
  test "does not render extra content when no block is given" do
    actual = render_inline(AtomicView::Components::EmptyStateComponent.new(title: "No messages")).to_html.strip
  ...
  test "merges custom class and forwards other options" do
    actual = render_inline(
      AtomicView::Components::EmptyStateComponent.new(title: "No messages", class: "custom-empty-state", id: "inbox-empty-state")
    ).to_html
